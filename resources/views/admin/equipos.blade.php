...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <!-- Filtros -->
    <form method="GET" action="{{ route('admin.equipos') }}" class="bg-white rounded-2xl shadow-sm p-6 mb-8 border border-gray-100">
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Buscar</label>
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Nombre del equipo..." class="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-gray-900 focus:border-transparent transition-all text-sm">
            </div>
            <div>
                ...
                <select name="event_id" class="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-gray-900 focus:border-transparent transition-all text-sm">
                    ...
                    <option value="{{ $evento->id }}" {{ request('event_id') == $evento->id ? 'selected' : '' }}>
                        {{ $evento->title }}
                    </option>
                    @endforeach
                </select>
        ...
        <div class="bg-white rounded-2xl shadow-sm overflow-hidden hover:shadow-lg transition-all duration-300 border border-gray-100">
            <div class="p-6">
                ...
                <div class="flex items-start justify-between mb-4">
                    <div class="flex-1">
                        <h3 class="text-xl font-bold text-gray-900 mb-1">{{ $equipo->name }}</h3>
                        <span class="px-2 py-1 bg-{{ $equipo->status === 'active' ? 'green' : 'gray' }}-100 text-{{ $equipo->status === 'active' ? 'green' : 'gray' }}-700 text-xs font-medium rounded-full">
                            {{ $equipo->status === 'active' ? 'Activo' : 'Inactivo' }}
                        </span>
                    </div>
                ...
                <p class="text-sm text-gray-600 mb-4">{{ Str::limit($equipo->description, 80) }}</p>
                @endif

                <!-- Información del Evento -->
                <div class="mb-4 pb-4 border-b border-gray-100">
                    <p class="text-xs text-gray-500 mb-1">Evento</p>
                    <p class="text-sm font-medium text-gray-900">{{ $equipo->event->title ?? 'Sin evento' }}</p>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <p class="text-sm font-medium text-gray-900">{{ $equipo->leader->name ?? 'Sin líder' }}</p>
                    </div>
                    <div>
                        ...
                        <div class="flex items-center gap-1">
                            ...
                            <span class="text-sm font-medium text-gray-900">{{ $equipo->members_count }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-blue-50 rounded-lg p-3 mb-4">
                    ...
                    <p class="text-sm font-semibold text-blue-900">{{ $equipo->project->title }}</p>
                    <span class="inline-block mt-1 px-2 py-0.5 bg-blue-100 text-blue-700 text-xs rounded-full">
                        {{ ucfirst($equipo->project->status) }}
                    </span>
                </div>
                @else
                ...
                <div class="flex items-center gap-2">
                    <button onclick="viewTeamDetails('{{ $equipo->id }}')" class="flex-1 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-100 rounded-lg transition-colors">
                        Ver Detalles
                    </button>
                    <button onclick="confirmDeleteTeam('{{ $equipo->id }}', '{{ $equipo->name }}')" class="px-4 py-2 text-sm font-medium text-red-700 hover:bg-red-50 rounded-lg transition-colors">
