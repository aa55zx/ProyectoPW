...
@section('title', 'Equipos Disponibles - Asesor')
...
<div class="p-8">
        ...
        <ol class="inline-flex items-center space-x-1">
            ...
            <li>
                <div class="flex items-center">
                    ...
                    <a href="{{ route('asesor.equipos') }}" class="text-sm font-medium text-gray-600 hover:text-gray-900">Equipos</a>
                </div>
            </li>
            <li>
                <div class="flex items-center">
    ...
    <div class="mb-8 flex items-center justify-between">
        ...
        <a href="{{ route('asesor.equipos') }}" class="px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors">
    ...
    @if($proyectosDisponibles->count() > 0)
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-200 hover:shadow-md transition-shadow">
            <div class="flex items-start justify-between mb-4">
                <div class="flex-1">
                    <h3 class="font-bold text-gray-900 text-lg">{{ $proyecto->team->name ?? 'Sin nombre' }}</h3>
                    @if($proyecto->event)
                    <p class="text-sm text-gray-600 mt-1">{{ $proyecto->event->title }}</p>
                    @endif
                </div>
            ...
            <div class="flex items-center gap-4 text-sm text-gray-600 mb-4">
                <div class="flex items-center gap-1">
                    ...
                    <span>{{ $proyecto->team->members_count ?? 0 }} miembros</span>
                </div>
                <div class="flex items-center gap-1">
            ...
            @if(in_array($proyecto->id, $misSolicitudesEnviadas))
                <button disabled class="w-full px-4 py-2 bg-gray-300 text-gray-600 rounded-lg cursor-not-allowed text-sm font-medium">
                    ⏳ Solicitud Enviada
                </button>
            @else
                <button onclick="mostrarModalSolicitar('{{ $proyecto->id }}', '{{ $proyecto->team->name }}')" class="w-full px-4 py-2 bg-black text-white rounded-lg hover:bg-gray-800 transition-colors text-sm font-medium">
                    Solicitar Asesorar
                </button>
            @endif
        </div>
